...
def get_radius_erode(image=None,shimg=None,Train=False):
    ...
    cv2.createTrackbar('Weight','image',200,2000, callback)
    ...
    while(1):
            ...
            dial = cv2.getTrackbarPos('dilation','image')
            ...
            thresh00 = np.uint8(cv2.erode(gray_frame, kernel,iterations=erode))
            thresh01 = cv2.dilate(thresh00,kernel,iterations=dial)
            thresh12 = cv2.distanceTransform(thresh01,cv2.DIST_L2,3)
            ret,thresh10 = cv2.threshold(thresh12,oldthresh,255,cv2.THRESH_BINARY)
            # The next is a dummy operation
            thresh1 = np.uint8(cv2.erode(thresh10, kernel,iterations=1))
    ...
    print("Your required thresholds are ",thresh)
...
def download_file_from_google_drive(id, destination):
    ...
    if (response.status_code == 200):
        print("Found CASSPER2 Data File!")
        ...
        print("Could not find CASSPER model files. Please email to nsp@airis4d.com!")
