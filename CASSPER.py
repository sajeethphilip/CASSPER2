...
def get_radius_erode(image=None):
    ...
    while(1):
            ...
            if k== ord('d'):
                if oldy<sy+Window_Size:
                    oldy=oldy+Window_Size
                    print(oldy)
                    img=imx[oldx-Window_Size:oldx+Window_Size,oldy-Window_Size:oldy+Window_Size]
            if k== ord('r'):
                if oldx<sx+Window_Size:
...
def PredictMRC(image=None,model_path='models/',model_name="CrossPro.ckpt",crop_height=512,crop_width=512,model="FRRN-B",dataset="./png/",mrcsrc="./mrc/",Savestar=True,CrossMatch=False,Box=True):
    ...
    print("------------------------------------------------------------------------------------------------------")
    print("Command is PredictMRC(image=None,model_path='/models/',model_name='BestFr_InceptionV4_model_FRRN-B.ckpt',crop_height=512,crop_width=512,model='FRRN-B',dataset='./png/',mrcsrc='./mrc/',Savestar=True,CrossMatch=False,Box=True)")
    print("------------------------------------------------------------------------------------------------------")
    print("")
...
def TrainMRC(num_epochs=300,epoch_start_i=0,checkpoint_step=5,validation_step=1,image=None,continue_training=False,dataset='./png',model_path='models/',TrDFolder='./mrc',batch_size=1,num_val_images=20,h_flip=False,v_flip=False,brightness=None,rotation=None):
    ...
    try:
        ...
        print('checkpoints folder exisits')
    os.mkdir(dataset)
